 ...
 * This should normally be the biggest type supported by a single load and store. 
 */ 
...
#ifndef MEM_COPY_BWD 
static __always_inline void mem_copy_bwd(unsigned long dstp, unsigned long srcp, size_t count) 
{ 
    srcp += count; 
    dstp += count; 
     /* If there not too few bytes to copy, use word copy. */ 
